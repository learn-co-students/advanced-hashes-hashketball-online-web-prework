...
def big_shoe_rebounds
  max_shoe_size = 0
  rebounds_by_shoe = 0
  game_hash.each_value { |teams|
    teams[:players].each { |player|
       player.each_value { |stats|
...
def winning_team
  home_team_score = 0
  away_team_score = 0
  game_hash[:home][:players].each { |player|  player.each_value { |stats|  home_team_score += stats[:points]} }
