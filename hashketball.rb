...
def team_names
  game_hash.map do |teams, team_hash|
    team_hash[:team_name]
  end
end
...
def player_numbers(team_name)
  jersey_numbers = []
   game_hash.collect do |team, jersey|
     if jersey[:team_name] == team_name
       jersey[:players].each do |player|
  ...
  player_rebounds = 0
    game_hash.each do | team, values|
      values[:players].each do | stats |
        if stats[:shoe] > largest_shoe_guy
          largest_shoe_guy = stats[:shoe]
