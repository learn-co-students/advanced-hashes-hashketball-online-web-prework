...
def num_points_scored(player)
  game_hash[:home][:players].each do |name, stats|
    stats.each do |type, value|
      if name.downcase == player.downcase && type == :points
        return value
        end
      end
    end
...
def shoe_size(player)
  game_hash[:home][:players][:names].each do |name, stats|
    stats.each do |type, value|
      if name.downcase == player.downcase && type == :shoe
        return value
        end
      end
    end
...
def team_colors(team)
  if team.downcase == game_hash[:home][:team_name].downcase
    return game_hash[:home][:colors]
  elsif team.downcase == game_hash[:away][:team_name].downcase
...
def player_stats(player)
  game_hash[:home][:players].each do |name, stats|
      if name.downcase == player.downcase
        return stats
        end
      end
      game_hash[:away][:players].each do |name, stats|
...
def player_numbers(team)
  team_numbers = []
  game_hash.each do |name, stats|
    if game_hash[name][:team_name] == team
      game_hash[name][:players].each do |player, data|
